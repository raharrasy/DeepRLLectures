...
if __name__ == "__main__":
	...
	for episode in range(numEpisodes):
		...
		if episode % 1000 == 0:
			torch.save(value_network.state_dict(), "DDQNParams/{}".format(str(episode//1000)))
		total_reward = 0.0
		if episode % 1000 == 0:
				env.render()
		while not done:
			...
			if total_actions > batch_size:
				...
				max_indexes = torch.max(compute_val(value_network, nextStateTensors), dim=1, keepdim=True)[1]
				target_next_state = compute_val(target_value_network, nextStateTensors).gather(1,max_indexes)
	...
	torch.save(value_network.state_dict(), "DDQNParams/{}".format(str(10)))
